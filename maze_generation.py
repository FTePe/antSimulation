...
def tower_hanoi(size,name='',flip=False): #Size is the number of pieces to move (must be int>=1)
    ...
    if size == 1:
        ...
        tl = Path(name+tl, 3+4, -(30-180*flip), -180*flip)  
        tr = Path(name+tr, 3+4, 30-180*flip, -180*flip)
        lr = Path(name+lr, 3+3, 30-180*flip, 180-30-180*flip)
        ...
        tl = Path(name+tl+str(size), 3+4, -(30-180*flip), -180*flip)
        tr = Path(name+tr+str(size), 3+4, 30-180*flip, -180*flip)
        lr = Path(name+lr+str(size), 3+3, 30-180*flip, 180-30-180*flip)
...
def mirror_hanoi(size):
    ...
    link1 = Path('link1', 3+4+3, -30, 30)
    link2 = Path('link2', 3+4+3, 30, -30)
...
def alternative_maze(size):
    ...
    #Defining the linking intersections
    mid_up_mid_down = Path('mid_up_mid_down',4,0,0)
    mid_up_left = Path('mid_up_left',3,-150,-150)
    mid_up_right = Path('mid_up_right',3,150,150)
    
    mid_down_left = Path('mid_down_left',3,-30,-30)
    mid_down_right = Path('mid_down_right',3,30,30)
